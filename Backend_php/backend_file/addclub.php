...
if(isset($postdata) && !empty($postdata)){
    ...
    $select = " SELECT * FROM addclub WHERE clubname = '$clubname' ";
    $result = mysqli_query($conn, $select);
    if(mysqli_num_rows($result) > 0){

        echo json_encode(array(
            "Error" => 'Error to join a club'
        ));
  
     }else{
        $insert = "INSERT INTO addclub(clubname, description) VALUES('$clubname','$description')";
        if(mysqli_query($conn, $insert)){
            http_response_code(201);
            echo json_encode(array(
                "Success" => 'Successfully join a club !'
            ));
