...
function Deleteclub() {
  ...
  useEffect(() => {
    fetch("http://localhost/backend_react/displayclub.php")
      .then((res) => res.json())
      .then((result) => {
        setItem(result);
        console.log(result);
  ...
  const deleteRow = async (clubname) => {
    ...
    axios.post('http://localhost/backend_file/deleteclub.php', config)
        .then((response) => {
  ...
  return (
    <main>
      ...
      <>
        <h1 className="products-heading">Deleteclub</h1>
          ...
          {item.map((item) => (
            <div className="card">
              ...
              <div className="content">
                ...
                <button onClick={() => deleteRow(item.clubname)}>Delete this club</button>
              </div>
            </div>
          ))}
        </div>
