...
if(isset($postdata) && !empty($postdata)){
    ...
    $select = " SELECT * FROM postadv WHERE title = '$title' ";
    $result = mysqli_query($conn, $select);
    if(mysqli_num_rows($result) > 0){

        echo json_encode(array(
            "Error" => 'Error to post an advertisement'
        ));
  
     }else{
        $insert = "INSERT INTO postadv(title, description) VALUES('$title','$description')";
        if(mysqli_query($conn, $insert)){
            http_response_code(201);
            echo json_encode(array(
                "Success" => 'Successfully posted an advertisement !'
            ));
