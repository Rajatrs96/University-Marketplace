...
if(isset($postdata) && !empty($postdata)){
    ...
    $select = " SELECT * FROM leaveclub WHERE clubname = '$clubname' ";
    $result = mysqli_query($conn, $select);
    if(mysqli_num_rows($result) > 0){

        echo json_encode(array(
            "Error" => 'Error to move data'
        ));
  
     }else{
        $insert = "INSERT INTO leaveclub(clubname, description) VALUES('$clubname','$description')";
        if(mysqli_query($conn, $insert)){
            http_response_code(201);
            echo json_encode(array(
                "Success" => 'Successfully moved datalo !'
            ));
